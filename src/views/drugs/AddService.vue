    ...
    <div class="form-row">
      <div class="form-group col-md-10">
        <label for="inputCity">Service Category </label>
            <select class="form-control"  v-model="register.service_category_id" @change="getCategories">
      ...
      <div class="form-group floating-label col-md-6">
          ...
          <input type="text" required class="form-control" placeholder="General Description" v-model="register.description">
      </div>
        ...
        <div class="table-responsive">
           <table class="table align-td-middle table-card">
               ...
               <tr v-for="service in services.data" v-bind:key="service.id">
                   ...
                   <td>
                     <button type="button" class="btn btn-info" @click="editService(service)"> <i class="fe fe-edit"></i></button>
                     <button style="margin-left:10px;" class="btn btn-danger" @click="deleteService(service)"><i class="fe fe-delete"></i></button>
                    </td>
               </tr>
               </tbody>
           </table>
  ...
  methods:{
    ...
    deleteService(service){
      if (confirm('Are you sure you want to delete Service?') ) {
        this.axios.delete(`/api/v1/auth/services/${service.id}`)
                    .then(response => {
                        console.log(response)
                        this.getServices()
    ...
    addService(){
        this.isLoading = true;
        this.user = JSON.parse(localStorage.getItem('user'))
        this.axios.post('/api/v1/auth/services',{
          agency_id : 95930,
    ...
    updateService(){
        this.isLoading = true;
        this.user = JSON.parse(localStorage.getItem('user'))
        this.axios.put(`/api/v1/auth/services/${this.service_id}`,{
          agency_id : 95930,
          service_category_id : this.register.service_category_id,
          price : this.register.price,
          kgshia_code : 'ZCHEMA',
        ...
        .then(response=>{
            ...
            this.edit = false;
            this.getServices()
            this.$toasted.success('Updated Successfully!', {position: 'top-left', duration:5000 })
