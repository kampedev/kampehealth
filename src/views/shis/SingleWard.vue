                   ...
                   <div class="col-lg-12">
                       <div class="card m-b-30">
                         <div class="card-header text-center">
                           ...
                           <div class="card-body">
                             <div class="col-md-12">
                               <p class="h5">Basic Healthcare Provision Fund: {{clients.bhcpf}}</p>
                               <p class="h5">State Equity Program: {{clients.sep}}</p>

                             </div>
                             <button class="btn btn-outline-primary" @click="show = !show">Bulk Update Sector</button>
                             <div class="form-group col-md-8" v-show="show">
                               ...
                               <select class="form-control"  v-model="sector" >
                                   ...
                                   <option  value="Universal Basic Education Healthcare Plan">Universal Basic Education  Healthcare Plan</option>
                                  <option  value="Voluntary Contributors Healthcare Plan">Voluntary Contributors Healthcare Plan</option>
                                  ...
                                  <option  value="Oganized Private Sector Healthcare Plan">Oganized Private Sector Healthcare Plan</option>
                              </select>

                                <button type="button" class="btn btn-primary" @click="updateSector" style="margin-top:10px;">Update</button>
                                     ...
                                     <tr v-for="enrollee in clients.data" v-bind:key="enrollee.id">
                                          ...
                                          <td v-show="show">
                                            <div class="row form-check">
                                              <input class="form-check-input" v-model="checked"
                                               type="checkbox" :value="enrollee.id"
                                               :id="enrollee.id" >
...
export default {
  ...
  beforeMount(){
    ...
    this.axios.get(`/api/v1/auth/getuserbyward/${this.$route.params.id}`)
                .then(response => {
  ...
  methods:{
    ...
    if (this.checked.length < 1 || this.sector == '') {
      this.$toasted.error('No enrolleee or sector selected', {position: 'top-center', duration:3000 })

    }
    else {
      this.isLoading = true
        this.axios.patch(`/api/v1/auth/change-sector-bulk`,{
          sector: this.sector,
          id_list: this.checked,
                    ...
                    .then(response => {
                        console.log(response)
                        this.getUsers()
                        this.$toasted.success('Enrollees Changed Successfully', {position: 'top-center', duration:3000 })
